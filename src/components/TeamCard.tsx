...
// Scheda di un membro del team. `compact` mostra la card centrata con avatar usata nelle
// griglie; altrimenti mostra una riga orizzontale. Il link LinkedIn è nascosto quando il
// valore salvato è "#" (il segnaposto del seed), così non compaiono mai link vuoti.
...
export function TeamCard({ member, compact = false }: TeamCardProps) {
  const imgSrc = member.imageUrl || PLACEHOLDER;
  ...
  if (compact) {
    return (
      <div className="card card-hover" style={{ padding: "1.5rem", textAlign: "center" }}>
        ...
        <p style={{ fontSize: "0.8125rem", color: "var(--foreground-muted)", marginBottom: "0.875rem" }}>{member.role}</p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.375rem", justifyContent: "center", marginBottom: "0.875rem" }}>
          {(member.specializations ?? []).slice(0, 2).map((spec) => (
            <span key={spec} className="badge badge-navy" style={{ fontSize: "0.6875rem" }}>{spec}</span>
          ))}
        </div>
        {member.linkedIn && member.linkedIn !== "#" && (
          <a href={member.linkedIn} style={{ display: "inline-flex", alignItems: "center", gap: "0.3rem", fontSize: "0.8125rem", color: "var(--brand-navy)", textDecoration: "none", fontWeight: 500 }}>
    ...
    <div className="card" style={{ overflow: "hidden", display: "flex" }}>
      ...
      <div style={{ padding: "1.5rem", flex: 1 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "0.75rem", marginBottom: "0.5rem" }}>
          <div>
            <h3 style={{ fontSize: "1.0625rem", fontWeight: 600, color: "var(--foreground-default)", marginBottom: "0.125rem" }}>{member.name}</h3>
            <p style={{ fontSize: "0.8125rem", color: "var(--brand-teal)", fontWeight: 500 }}>{member.title} · {member.role}</p>
          </div>
          {member.linkedIn && member.linkedIn !== "#" && (
        ...
        <p style={{ fontSize: "0.875rem", lineHeight: 1.65, color: "var(--foreground-muted)", marginBottom: "1rem" }}>{member.bio}</p>
