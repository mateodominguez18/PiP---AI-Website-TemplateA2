...
import { ConsultingArea } from "@/data/mockData";
...
export function ConsultingCard({ area, variant = "grid" }: ConsultingCardProps) {
  ...
  if (variant === "list") {
    return (
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <h3 className="card-title" style={{ color: "var(--foreground-default)", marginBottom: "0.5rem" }}>
            {area.title}
          </h3>
          <p style={{ fontSize: "0.875rem", lineHeight: 1.6, color: "var(--foreground-muted)", marginBottom: "0.875rem" }}>
            {area.shortDescription}
          </p>
          <Link
            href={`/consulenza/${area.slug}`}
            style={{
              display: "inline-flex",
              alignItems: "center",
              gap: "0.3rem",
  ...
  return (
    ...
    >
      ...
      <h3 className="card-title" style={{ color: "var(--foreground-default)", marginBottom: "0.625rem" }}>
        {area.title}
      </h3>
      <p style={{ fontSize: "0.875rem", lineHeight: 1.65, color: "var(--foreground-muted)", flex: 1, marginBottom: "1.25rem" }}>
        {area.shortDescription}
      </p>
      <Link
        href={`/consulenza/${area.slug}`}
