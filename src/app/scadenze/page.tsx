...
export default function ScadenzePage() {
  ...
  const filtered = taxDeadlines.filter((d) => {
    const matchSearch =
      !search ||
      d.title.toLowerCase().includes(search.toLowerCase()) ||
      d.description.toLowerCase().includes(search.toLowerCase()) ||
      d.audience.toLowerCase().includes(search.toLowerCase());
    const matchMonth = selectedMonth === "Tutti" || d.month === selectedMonth;
  ...
  const grouped: Record<string, typeof taxDeadlines> = {};
  filtered.forEach((d) => {
    ...
    <div>
      ...
      <section className="section">
        <div className="container">
          <div
            className="card"
            style={{ padding: "1.5rem", marginBottom: "2.5rem", display: "flex", flexWrap: "wrap", gap: "1rem", alignItems: "flex-end" }}
          >
          ...
          <div style={{ display: "flex", gap: "1.25rem", marginBottom: "2rem", flexWrap: "wrap" }}>
            {[
              { label: "Alta priorità", cls: "badge-red" },
              { label: "Media priorità", cls: "badge-teal" },
              { label: "Bassa priorità", cls: "badge-gray" },
            ].map((item) => (
              <div key={item.label} style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                <span className={`badge ${item.cls}`} style={{ fontSize: "0.6875rem" }}>
            ...
            Object.entries(grouped).map(([month, deadlines]) => (
              <div key={month} style={{ marginBottom: "2.5rem" }}>
                ...
                >
                  ...
                  <h2 style={{ fontSize: "1.125rem", fontWeight: 700, color: "var(--brand-navy)" }}>{month} 2025</h2>
                  <span className="badge badge-navy" style={{ marginLeft: "auto" }}>
                    {deadlines.length} scadenz{deadlines.length === 1 ? "a" : "e"}
                  </span>
                </div>
                  ...
                  <table className="deadline-table" style={{ width: "100%" }}>
                    ...
                    <tbody>
                      {deadlines.map((d) => (
                        <tr key={d.id}>
                          <td>
                            <span style={{ fontWeight: 600, color: "var(--foreground-default)", fontSize: "0.9rem" }}>
                              {d.date}
                            </span>
                          </td>
                          <td>
                            <p style={{ fontWeight: 500, fontSize: "0.9rem", color: "var(--foreground-default)", marginBottom: "0.25rem" }}>
                              {d.title}
                            </p>
                          ...
                          <td>
                            <span className="badge badge-gray" style={{ fontSize: "0.6875rem" }}>{d.category}</span>
                          </td>
                          <td>
                            <span style={{ fontSize: "0.8125rem", color: "var(--foreground-muted)" }}>{d.audience}</span>
                          </td>
                          <td>
                            <span
                              className={`badge ${d.priority === "alta" ? "badge-red" : d.priority === "media" ? "badge-teal" : "badge-gray"}`}
          ...
          <div
            className="card"
            style={{ padding: "1.5rem", backgroundColor: "var(--action-secondary)", border: "none", marginTop: "2rem" }}
          >
            <p style={{ fontSize: "0.875rem", color: "var(--brand-navy)", lineHeight: 1.6 }}>
              ...
              <Link href="/contatti" style={{ color: "var(--brand-navy)", textDecoration: "underline" }}>
                Richiedi assistenza →
              </Link>
            </p>
          </div>
