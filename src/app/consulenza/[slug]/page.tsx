...
export async function generateStaticParams() {
  return consultingAreas.map((area) => ({ slug: area.slug }));
}

export default async function ConsulenzaDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const area = consultingAreas.find((a) => a.slug === slug);

  if (!area) notFound();

  const otherAreas = consultingAreas.filter((a) => a.slug !== slug).slice(0, 3);
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "4rem", alignItems: "start" }} className="detail-grid">
            ...
            <div>
              <div style={{ marginBottom: "3rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.875rem", marginBottom: "1.25rem" }}>
                  ...
                  <h2 className="section-title" style={{ color: "var(--foreground-default)" }}>
                    Cosa facciamo
                  </h2>
                </div>
                <p style={{ fontSize: "1.0625rem", lineHeight: 1.75, color: "var(--foreground-muted)" }}>
                  {area.fullDescription}
                </p>
              </div>

              <div style={{ marginBottom: "3rem" }}>
                <h3 className="section-title" style={{ color: "var(--foreground-default)", marginBottom: "1.25rem" }}>
                  Attività principali
                </h3>
                <div
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                  {area.benefits.map((benefit, i) => (
                    ...
                    >
                      ...
                      <p style={{ fontSize: "0.9375rem", lineHeight: 1.6, color: "var(--foreground-default)" }}>
                        {benefit}
                      </p>
                    </div>
                  ))}
            ...
            <div style={{ position: "sticky", top: "5.5rem", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
              <div
                ...
                style={{
                  padding: "1.75rem",
                  background: `linear-gradient(160deg, var(--brand-navy-dark) 0%, var(--brand-navy) 100%)`,
                  border: "none",
                }}
              >
                <h3 style={{ fontSize: "1.125rem", fontWeight: 600, color: "white", marginBottom: "0.75rem" }}>
                  Serve assistenza su {area.title.toLowerCase()}?
                </h3>
                <p style={{ fontSize: "0.875rem", lineHeight: 1.65, color: "rgba(255,255,255,0.7)", marginBottom: "1.25rem" }}>
                  Contattaci per una prima valutazione. Un professionista dedicato ti risponderà entro 24 ore.
                </p>
              ...
              <div className="card" style={{ padding: "1.5rem" }}>
                ...
                <p style={{ fontSize: "0.9rem", color: "var(--foreground-default)", lineHeight: 1.6 }}>
                  {area.targetClients}
                </p>
              </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.625rem" }}>
                  {otherAreas.map((a) => (
                    <Link
                      ...
                      style={{ display: "flex", alignItems: "center", gap: "0.5rem", textDecoration: "none", color: "var(--foreground-default)", fontSize: "0.875rem", padding: "0.375rem 0", borderBottom: "1px solid var(--border-default)" }}
                    >
                      <ArrowRight size={13} style={{ color: "var(--brand-teal)", flexShrink: 0 }} />
                      {a.title}
                    </Link>
                  ))}
                  <Link href="/consulenza" style={{ fontSize: "0.8125rem", color: "var(--brand-navy)", textDecoration: "none", fontWeight: 500, marginTop: "0.25rem" }}>
                    Vedi tutte →
                  </Link>
                </div>
              </div>
